...
### 普通用户使用文件传输 | File Transfer for Users

普通用户可以通过以下方式传输文件，无需编写代码：

1. **拖拽上传** - 直接拖拽文件到终端窗口即可上传
2. **文件浏览器** - 点击工具栏的文件传输按钮打开文件浏览器
3. **右键菜单** - 右键点击文件选择上传/下载

> **注意**: 远程服务器需要安装 Kitty 的 `ki` 工具才能接收文件

### 开发者 API | API Examples

以下是如何在代码中使用各个 Kitty 协议服务的示例（仅供开发者参考）：
