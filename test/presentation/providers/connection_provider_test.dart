...
void main() {
  ...
  setUp(() {
    ...
    container = ProviderContainer(
      overrides: [connectionRepositoryProvider.overrideWithValue(mockRepo)],
    );
  });
  ...
  group('ConnectionNotifier', () {
    group('initial state', () {
      test(
        'Given new provider, When created, Then has loading state with empty connections',
        () {
      ...
      test(
        'Given same fields, When comparing states, Then equal and same hashCode',
        () {
          final connections = [
            SshConnection(
              id: 'conn1',
          ...
          final a = ConnectionState(
            error: 'err',
            searchQuery: 'q',
            connections: connections,
          );
          final b = ConnectionState(
            error: 'err',
            searchQuery: 'q',
            connections: connections,
          );
    ...
    group('deleteConnection', () {
      test(
        ...
        () async {
          ...
          when(
            () => mockRepo.deleteConnection(connectionId),
          ).thenAnswer((_) async {});
    ...
    group('getConnectionById', () {
      test(
        ...
        () {
          ...
          when(
            () => mockRepo.getConnectionById(connectionId),
      ...
      test(
        ...
        () {
          ...
          when(() => mockRepo.getConnectionById(connectionId)).thenReturn(null);
    ...
    group('search and filter', () {
      ...
      test(
        'Given clearSearch called, When accessed, Then searchQuery is empty',
        () {
          // Arrange
    ...
    group('duplicate connection handling', () {
      test(
        ...
        () async {
          ...
          when(
            () => mockRepo.saveConnection(connection1),
          ).thenAnswer((_) async {});
          ...
          when(
            () => mockRepo.saveConnection(connection2),
          ).thenAnswer((_) async {});
          when(
            () => mockRepo.getAllConnections(),
