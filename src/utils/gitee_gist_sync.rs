...
impl Default for GiteeGistConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            gist_id: String::new(),
...
/// Gitee Gist 同步服务
pub struct GiteeGistSyncService {
    connections_path: PathBuf,
    config_path: PathBuf,
}

impl GiteeGistSyncService {
    /// API 基地址
    const API_BASE: &'static str = "https://gitee.com/api/v5";

    /// 创建新服务
    pub fn new(connections_path: PathBuf, config_dir: PathBuf) -> Self {
        let config_path = config_dir.join("gitee_gist_sync.json");
        Self {
            connections_path,
            config_path,
        }
    ...
    /// 加载配置
    pub fn load_config(&self) -> GiteeGistConfig {
        ...
        GiteeGistConfig::default()
    }

    /// 保存配置
    pub fn save_config(&self, config: &GiteeGistConfig) -> Result<(), String> {
    ...
    /// 获取远程 Gist 内容
    async fn fetch_remote_gist(&self, config: &GiteeGistConfig) -> Result<Option<GistResponse>, String> {
        if config.gist_id.is_empty() {
            return Err("Gist ID 为空".to_string());
        }

        let client = self.get_client()?;
        let url = format!("{}/gists/{}", Self::API_BASE, config.gist_id);
        ...
        if !status.is_success() {
            ...
            return Err(format!("Gitee API 错误: {} - {}", status, error_text));
        }
    ...
    /// 上传到 Gist
    pub async fn upload_to_gist(&self, config: &GiteeGistConfig) -> Result<GistSyncResult, String> {
        if !config.enabled {
            return Err("Gitee Gist 同步未启用".to_string());
        }
    ...
    /// 从 Gist 下载
    pub async fn download_from_gist(&self, config: &GiteeGistConfig) -> Result<GistSyncResult, String> {
        if !config.enabled {
            return Err("Gitee Gist 同步未启用".to_string());
        }
    ...
    /// 双向同步
    pub async fn sync_bidirectional(&self, config: &GiteeGistConfig) -> Result<GistSyncResult, String> {
        if !config.enabled {
            return Err("Gitee Gist 同步未启用".to_string());
        }
    ...
    pub fn get_gist_url(&self) -> String {
        ...
        if config.gist_id.is_empty() {
            ...
            format!("https://gitee.com/gist/{}", config.gist_id)
        }
    }
}
...
impl GiteeGistSyncService {
    /// 检测冲突详情
    pub async fn detect_conflicts(&self, config: &GiteeGistConfig) -> Result<Vec<ConflictDetail>, String> {
        let local_connections = self.load_local_connections()?;
        let gist = match self.fetch_remote_gist(config).await? {
            Some(g) => g,
